...
const EMOJIS = {
  ...
  PESQUISA: '<:search:1539125799947468890>',
  REGRA: '<:rule:1539125787158908928>',
  CARREGANDO: '<a:loanding:1534612861211377868>',
  ...
  PROTECAO2: '<:proteo:1539125790711480331>',
...
client.on(Events.MessageCreate, async message => {
    ...
    const menu = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        ...
        .addOptions([
          ...
          new StringSelectMenuOptionBuilder().setLabel(`${EMOJIS.LOGS} Canal de Logs`).setValue('set_log_channel'),
          ...
          new StringSelectMenuOptionBuilder().setLabel(`${EMOJIS.LINK_EXTERNO} URL do Tribunal`).setValue('set_tribunal_url'),
          new StringSelectMenuOptionBuilder().setLabel(`${EMOJIS.STAFF} Permissões`).setValue('set_perms'),
  ...
  if (cmd === 'mute') {
    ...
    const btn = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`mute_confirm_${user.id}_${tempo}`).setLabel(`${EMOJIS.ACEITAR} Confirmar`).setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`mute_cancel_${user.id}`).setLabel(`${EMOJIS.RECUSAR} Cancelar`).setStyle(ButtonStyle.Secondary)
    );
    return message.reply({ embeds: [confirmEmbed], components: [btn] });
  }
});
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'painel_menu') {
    ...
    if (value === 'set_tribunal_url') {
      const modal = new ModalBuilder()
        ...
        .setTitle(`${EMOJIS.LINK_EXTERNO} URL do Tribunal`)
        .addComponents(
    ...
    if (value === 'set_prefix') {
      const modal = new ModalBuilder()
        ...
        .setTitle(`${EMOJIS.ENGRENAGEM} Novo Prefixo`)
        .addComponents(
          new ActionRowBuilder().addComponents(
...
client.on('ready', async () => {
  console.log(`${EMOJIS.ZYPHOR} Comandos de barra registrados!`);
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (commandName === 'mute') {
    ...
    const btn = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`mute_confirm_${alvo.id}_${tempo}`).setLabel(`${EMOJIS.ACEITAR} Confirmar`).setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`mute_cancel_${alvo.id}`).setLabel(`${EMOJIS.RECUSAR} Cancelar`).setStyle(ButtonStyle.Secondary)
    );
    return interaction.reply({ embeds: [embed], components: [btn] });
  }
  ...
  if (commandName === 'unmute') {
    ...
    if (!membro) return interaction.reply(`${EMOJIS.VERMELHO} Usuário não encontrado!`);
    const muteRole = guild.roles.cache.find(r => r.name === 'Muted');
    if (!muteRole || !membro.roles.cache.has(muteRole.id)) {
      return interaction.reply(`${EMOJIS.ALERTA} Usuário não está mutado!`);
    }
    await membro.roles.remove(muteRole.id);
    return interaction.reply(`${EMOJIS.VERDE} Usuário \
