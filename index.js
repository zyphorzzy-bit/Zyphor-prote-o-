...
// ==============================================
// 🎨 EMOJIS PERSONALIZADOS — ZYPHOR (CORRIGIDOS)
// ==============================================
const EMOJIS = {
...
// ==============================================
// 🤖 INICIALIZAR BOT — COMPONENTES V2
// ==============================================
...
client.on(Events.MessageCreate, async message => {
  ...
  if (cmd === 'painel' || cmd === 'config') {
    if (!isOwner(message.author.id, message.guild.id)) {
      return message.reply(`${EMOJIS.VERMELHO} **SÓ O DONO PODE CONFIGURAR O BOT!**`);
    }
    const cfg = getConfig(message.guild.id);
    ...
    const menu = new StringSelectMenuBuilder()
      ...
      .addOptions([
        ...
        new StringSelectMenuOptionBuilder().setLabel('🛡️ Cargo Protegido').setValue('set_protected_role'),
        ...
        new StringSelectMenuOptionBuilder().setLabel('💥 Anti-Nuke').setValue('set_anti_nuke'),
        new StringSelectMenuOptionBuilder().setLabel('🛡️ Permissões de Comandos').setValue('set_command_perms'),
        new StringSelectMenuOptionBuilder().setLabel('📺 Streaming').setValue('set_stream')
      ]);
  ...
  if (cmd === 'prefix' || cmd === 'setprefix') {
    if (!isOwner(message.author.id, message.guild.id)) {
      return message.reply(`${EMOJIS.VERMELHO} Apenas o dono pode mudar o prefixo!`);
    }
    const novo = args[0];
    if (!novo || novo.length < 2) {
      return message.reply(`${EMOJIS.AMARELO} Use: ${prefix}prefix <letra+simbolo> | Exemplo: s!prefix k!`);
    }
    cfg.PREFIX_LETTER = novo[0];
    cfg.PREFIX_SYMBOL = novo[1] || '!';
    saveData();
...
// ==============================================
// 🎯 INTERAÇÕES — BOTÕES CINZA (Secondary)
// ==============================================
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'painel_menu') {
    if (!isOwner(interaction.user.id, interaction.guild.id)) {
      return interaction.reply({ content: `${EMOJIS.VERMELHO} SÓ O DONO PODE CONFIGURAR!`, ephemeral: true });
    }
    ...
    if (value === 'set_protected_role') {
      ...
      const menu = new StringSelectMenuBuilder()
        ...
        .addOptions([
          new StringSelectMenuOptionBuilder().setLabel('❌ Desativar').setValue('none'),
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'ban') {
      if (!canUseCommand(member, guild.id, 'ban')) {
        return interaction.reply({ content: `${EMOJIS.VERMELHO} Sem permissão para usar /ban!`, ephemeral: true });
      }
      ...
      // 🔘 BOTÕES CINZA — Secondary
      const btn = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`confirm_ban_${alvo.id}`).setLabel('✅ Confirmar').setStyle(ButtonStyle.Secondary),
    ...
    if (commandName === 'kick') {
      if (!canUseCommand(member, guild.id, 'kick')) {
        return interaction.reply({ content: `${EMOJIS.VERMELHO} Sem permissão para usar /kick!`, ephemeral: true });
      }
      const alvo = options.getUser('usuário');
      const motivo = options.getString('motivo') || 'Sem motivo';
      const membro = guild.members.cache.get(alvo.id);
      ...
      // 🔘 BOTÕES CINZA — Secondary
      const btn = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`confirm_kick_${alvo.id}`).setLabel('✅ Confirmar').setStyle(ButtonStyle.Secondary),
    ...
    if (commandName === 'mute') {
      if (!canUseCommand(member, guild.id, 'mute')) {
        return interaction.reply({ content: `${EMOJIS.VERMELHO} Sem permissão para usar /mute!`, ephemeral: true });
      }
      const alvo = options.getUser('usuário');
      const motivo = options.getString('motivo') || 'Sem motivo';
      ...
      // 🔘 BOTÕES CINZA — Secondary
      const btn = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`confirm_mute_${alvo.id}_${tempo}`).setLabel('✅ Confirmar').setStyle(ButtonStyle.Secondary),
...
client.on('ready', async () => {
  console.log(`${EMOJIS.ZYPHOR} Comandos registrados!`);
